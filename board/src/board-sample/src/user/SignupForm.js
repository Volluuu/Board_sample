...
function SignupForm(props) {
  ...
  const [hpErrorMsg, setHpErrorMsg] = useState(
    '유효하지 않는 번호 입니다. ("-"를 포함하여 입력해주세요)'
  );
  ...
  //아이디 중복체크
  const idCheck = (id) => {
    let idCherckUrl = process.env.REACT_APP_URL + "/user/idcheck?id=" + id;
    axios.get(idCherckUrl).then((res) => {
      if (res.data > 0) {
  ...
  const emailCheck = (email) => {
    let emailCheckUrl =
      process.env.REACT_APP_URL + "/user/emailcheck?email=" + email;
    axios.get(emailCheckUrl).then((res) => {
      if (res.data > 0) {
        setEmailErrorMsg("이미 사용 중인 이메일 입니다");
        setEmailError(true);
        setEmailSendBtn(true);
  ...
  const emailSendBtnEvent = () => {
    ...
    axios.post(emailSendUrl, { email }).then((res) => {
    ...
    axios
      .post(
        signupUrl,
        {
          ...
          addr: addrRef.current.value.concat(
    ...
    <div className="container join" data-v-6ca47fe2="" data-v-3007c576="">
      <div className="content lg" data-v-6ca47fe2="">
        <div className="join_area" data-v-6ca47fe2="">
          <h2 className="join_title" data-v-6ca47fe2="">
            Sign Up
          </h2>
          <form onSubmit={onSubmitSignUp}>
            ...
            >
              ...
              <div className="input_item" data-v-6c561060="">
                <input
                  type="text"
                  placeholder="사용하실 ID를 입력해주세요(10글자 이내) "
                  autoComplete="off"
                  ref={idRef}
                  className="input_txt"
                  data-v-6c561060=""
              ...
              <p className="input_error" data-v-6c561060="" data-v-6ca47fe2="">
                {hpErrorMsg}
              </p>
            </div>
            <div
            ...
            >
              ...
              <p className="input_error" data-v-6c561060="" data-v-6ca47fe2="">
                {" "}
                영문, 숫자, 특수문자를 조합하여 입력해주세요. (8-16자, 공백
                없이){" "}
              </p>
            </div>
            ...
            onClick={() => {
              Swal.fire({
                ...
                if (result.isConfirmed) {
                  Swal.fire("이메일 인증이 취소되었습니다.", "", "warning");
                  setEmailCodeModal("none");
                  emailCodeRef.current.value = "";
                }
              });
